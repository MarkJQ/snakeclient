...
function drawLeft() {
    ...
    var hexTileUp = "#e74c3c" + ("000000" + rgbToHex(tileUp[0], tileUp[1], tileUp[2])).slice(-6);
    var tileDown = ctx.getImageData(cx, cy + tile_size, tile_size, tile_size).data;
    var hexTileDown = "#e74c3c" + ("000000" + rgbToHex(tileDown[0], tileDown[1], tileDown[2])).slice(-6);
    var tileLeft = ctx.getImageData(cx - tile_size, cy, tile_size, tile_size).data;
    var hexTileLeft = "#e74c3c" + ("000000" + rgbToHex(tileLeft[0], tileLeft[1], tileLeft[2])).slice(-6);

    if (hexTileUp == "#e74c3c" && hexTileDown == "#e74c3c" && hexTileLeft == "#e74c3c") {
...
function drawUp() {
    if (cy < tile_size) {
        ...
        $("#notification").text("MOVE A LITTLE DOWN, JUST SAVED YA BRO.").fadeTo('slow', 1).fadeTo('slow', 0);
